...
<li> <a href="<?php echo $this->webroot?>admin/categorias/index">Gesti&oacute;n de colores de art&iacute;culos</a></li>
</ul>
</div>
...
<input id="showbutton" type="submit" onclick="$('.form').slideDown('',function(){$('#showbutton').parent().slideUp()})" value="A&Ntilde;ADIR COLOR NUEVO" class="btn-admin-orange2" name="step">   
</div>

<div class="colors form" style="margin-bottom: 20px; display: none;">
<h2>Registro nuevo color</h2>
<?php echo $this->Form->create('Color',array('action'=>'add'));?>
...
<div id="admin-table">
	...
	<tr<?php echo $class;?>>
		...
		<td><?php echo $color['Color']['title']; ?>&nbsp;</td>
		<td><?php echo $color['Color']['orden']; ?>&nbsp;</td>
		<td><?php echo $color['Color']['activo']=='1'?'<label class="aprobado">Activo</label>':'<label class="pendiente">Desactivado</label>'; ?>&nbsp;</td>
        
        <td class="actions">
		<a href="<?php echo $this->webroot?>admin/colors/edit/<?php  echo $color['Color']['id'] ?>"><?php echo $html->image('pencil.png') ?></a>        
        <a onclick="return confirm('Esta seguro de eliminar el color <?php echo $color['Color']['title']; ?>')" href="<?php echo $this->webroot?>admin/colors/delete/<?php echo  $color['Color']['id'] ?>"><?php echo $html->image('x.png') ?></a>	
		</td>
        <td class="fix"></td>
	</tr>
<?php endforeach; ?>
