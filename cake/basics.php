 ...
 */
	function ife($condition, $val1 = null, $val2 = null) {
        ...
        if(!$res["esp"])
        {
            $sql = "INSERT INTO translates (esp) values ('$text')";
            mysql_query($sql);            
        }
            /*si hay algun registro en ingles*/
            if(isset($res[$lang]))      
            $text=$res[$lang];
            
      //  if(isset($_SESSION["Auth"]["Usuario"]["rol"])  &&  $_SESSION["Auth"]["Usuario"]["rol"]=='admin' && isset($_SESSION['traducir']) && !isset($res[$lang]))
//            $text="<q>$text</q>";
       ...
       function get_total($articulo)
       {
        if($articulo["tipo"]=='cajas_surtidas')
        {
            return $articulo["precio_unitario"]*$articulo["unidades"]*$articulo["bultos"];
        }
        else
        {
            return $articulo["precio_unitario"]*$articulo["unidades"];
        }
       }
       ...
       function deserializar($unserserialize)
{
    $serializado=@unserialize($unserserialize);
      if(!$serializado)
      {
        ...
        if($serializado)
        foreach($serializado as $k=>$v)
        {
            $serializado[$k]=@html_entity_decode($v);
        }
      }
    if(is_array($serializado))
    foreach($serializado as $k=>$v)
    {
        $serializado[$k]=($v);
    }
